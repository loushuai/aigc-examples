...
from utils.utils import from_flattened_numpy, to_flattened_numpy
...
def get_sampling_fn(method, sde, shape, inverse_scaler, eps, device='cuda'):
  ...
  if sampler_name.lower() == 'rectified_flow':
    sampling_fn = get_rectified_flow_sampler(sde=sde, shape=shape, inverse_scaler=inverse_scaler, device=device)
  else:
